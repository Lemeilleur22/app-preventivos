...
def enviar_reporte_semanal_supervisores():
    ...
    preventivos = supabase.table("preventivos") \
        .select("*") \
        .gte("schedfinish", inicio.isoformat()) \
        .lte("schedfinish", fin.isoformat()) \
        .execute().data
    ...
    pendientes = df[df["estatus"].isin(["PENDIENTE", "EN PROCESO"])]
    resumen = df.groupby(["tecnico", "estatus"]
    ...
    <h3>Preventivos asignados esta semana </h3>
    {df[["tecnico", "area", "numero_ot", "descripcion", "sede", "schedfinish", "estatus"]].to_html(index=False)}
    
    <h3>Preventivos pendientes </h3>
    {pendientes[["tecnico", "area", "numero_ot", "descripcion", "sede", "schedfinish", "estatus"]].to_html(index=False)}
    """
            ...
            for _, row in df.iterrows():
                ...
                # -----------------
                    ...
                    candidatos = [
                        ...
                        and limpiar_turno(t.get("turno_actual")) == "3"
                        and t["id"] != tecnico_cba["id"]
                    ]

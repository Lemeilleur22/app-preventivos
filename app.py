...
def mostrar_checklist_arranque(ot_data, solo_lectura=False, fecha_checklist=None):
    ...
    with st.form(f"form_checklist_arranque_{numero_ot}_{fecha_checklist}"):
        for item in ITEMS_CHECKLIST_ARRANQUE:
        ...
        guardar = st.form_submit_button("Guardar checklist", width="stretch")

    if guardar:
        incompletos = [r["item"] for r in registros if not r["resultado"]]

        if incompletos:
            st.error("Debes marcar OK o Requiere atención en todos los equipos.")
            st.stop()

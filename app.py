    ...
    with tab4:
        ...
        if archivo_jp:
            df_excel = pd.read_excel(
                archivo_jp,
                sheet_name="Job Plan Detail Preview",
                header=None
            )
            ...
            if inicio_tasks:
                for i in range(inicio_tasks, len(df_excel)):
                    secuencia = df_excel.iloc[i, 0]
                    descripcion = df_excel.iloc[i, 1]

                    if pd.notna(secuencia) and pd.notna(descripcion):
            ...
            if st.button("Guardar Job Plan"):
                ...
                if existente:
                    st.warning("Ese Job Plan ya existe.")
                else:
                    supabase.table("jobplan_tareas").insert(
                        df_tareas.to_dict(orient="records")
                    ).execute()
...
if modo == "🌎 General":
    ...
    if not df_prev.empty:
        ...
        jpnum_ot_raw = fila_ot.iloc[0]["jpnum"]

        try:
            jpnum_ot = str(int(float(jpnum_ot_raw)))
        except:
            jpnum_ot = str(jpnum_ot_raw).strip()
        ...
        if evidencia_existente and pd.notna(evidencia_existente):
            st.session_state.evidencia_url = evidencia_existente
            st.success("Esta OT ya tiene evidencia subida.")
            st.image(evidencia_existente, width=300)

...
if modo == "Admin":
    ...
    with tab6:
        ...
        if archivo_roles:
            df_raw = pd.read_excel(archivo_roles, header=None)
            fila_header = None

            for i, row in df_raw.iterrows():
                valores = [str(x).strip().upper() for x in row if pd.notna(x)]
            ...
            if fila_header is None:
                st.error("No se encontró encabezado con NOMBRE y TURNO")
                st.stop()

            df_roles = pd.read_excel(
                archivo_roles,
                header=fila_header
            )

            df_roles.columns = df_roles.columns.str.strip()
            st.write(f"Encabezado detectado en fila {fila_header + 1}")
            st.dataframe(df_roles.head(10), hide_index=True)

            if st.button("Actualizar turnos:"):
                actualizados = 0

                for _, row in df_roles.iterrows():
                    nombre_excel = str(row["NOMBRE"]).strip().upper()
                    ...
                    supabase.table("tecnicos").update({
                        "turno_actual": turno
                    }).eq("nombre_nomina", nombre_excel).execute()

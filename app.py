    ...
    with tab2:
        ...
        if not df_admin_prev.empty:
            ...
            if not fila.empty:
                ...
                anotaciones = fila.iloc[0].get("evidencias")
                fecha_inicio = fila.iloc[0].get("fecha_inicio")
                fecha_cierre = fila.iloc[0].get("fecha_fin")
            ...
            # Mostrar evidencia si existe
                st.markdown("---")
                st.subheader("Detalle de la ejecución")

                if fecha_inicio:
                    st.write(f"Inicio: {fecha_inicio}")
                if fecha_cierre:
                    st.write(f"Fin: {fecha_cierre}")

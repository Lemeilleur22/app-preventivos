...
def mostrar_detalle_ejecucion(fecha_inicio, fecha_cierre):
    if fecha_inicio is None or pd.isna(fecha_inicio):
        return

    try:
        inicio = pd.to_datetime(fecha_inicio, utc=True).tz_convert(
            "America/Mexico_City")
    except:
    ...
    if fecha_cierre is not None and pd.notna(fecha_cierre):
        try:
            cierre = pd.to_datetime(fecha_cierre, utc=True).tz_convert(
                "America/Mexico_City")
        except:
            ...
            if cierre >= inicio:
                ...
                fin_fmt = "Fecha invalida"
                duracion_fmt = "Error en timestamps"
    ...
    <div class="detalle-card">
        <div class="detalle-title"> Detalle de la ejecucion</div>

        <div class="detalle-box inicio">
            <div>
                <div class="detalle-label"> Inicio</div>
                <div class="detalle-value">{inicio_fmt}</div>
            </div>
            <div class="badge">{inicio.strftime("%d/%m/%Y")}</div>
        </div>

        <div class="detalle-box fin">
            <div>
                <div class="detalle-label"> Fin</div>
                <div class="detalle-value">{fin_fmt}</div>
            </div>
        </div>

        <div class="detalle-box duracion">
            <div>
                <div class="detalle-label"> Duracion</div>
                <div class="detalle-value">{duracion_fmt}</div>
            </div>
        </div>
    </div>
    """
...
if modo == "Admin":
    ...
    with tab1:
        ...
        if archivo:
            ...
            contador_fontaneria = 0
            LIMITE_FONTANERIA = random.randint(2, 3)
            ...
            for _, row in df.iterrows():
                ...
                # ----------------
                    ...
                    if contador_fontaneria < LIMITE_FONTANERIA and random.choice([True, False]):
                ...
                tecnico_asignado = random.choice(candidatos)

...
@st.cache_data(ttl=30)
def cargar_checklist_preventivo(numero_ot, tipo_checklist):
    data = (
...
def mostrar_checklist_arranque(ot_data, solo_lectura=False):
    numero_ot = str(ot_data.get("numero_ot", ""))
    tipo_checklist = "ARRANQUE_COMEDORES"

    guardados = cargar_checklist_preventivo(numero_ot, tipo_checklist)

    st.markdown("### Checklist de arranque a comedor")
    st.caption(f"OT: {numero_ot} | Job Plan: {obtener_jobplan_ot(ot_data)}")
    ...
    with st.form(f"form_checklist_arranque_{numero_ot}"):
        for item in ITEMS_CHECKLIST_ARRANQUE:
            actual = guardados.get(item, {})
            resultado_actual = actual.get("resultado")

            opciones = ["OK", "REQUIERE ATENCION"]
            ...
            with col1:
                resultado = st.radio(
                    ...
                    index=index_actual,
                    horizontal=True,
                    key=f"resultado_{numero_ot}_{item}"
                )
            with col2:
                observaciones = st.text_input(
                    "Observaciones",
                    value=actual.get("observaciones", "") or "",
                    key=f"obs_{numero_ot}_{item}",
    ...
    if guardar:
        ...
        supabase.table("checklist_preventivos").upsert(
            registros,
            on_conflict="numero_ot,tipo_checklist,item"
        ).execute()
            ...
            if not solo_lectura:
                ...
                if es_checklist_arranque(ot_data):
                    mostrar_checklist_arranque(
                        ot_data, solo_lectura=solo_lectura)
                else:
        ...
        if termino:
            ...
            if not isinstance(rango_auditoria, tuple) or len(rango_auditoria) != 2:
                ...
                if not fila.empty:
                    ...
                    with st.expander("Detalles de tareas", expanded=True):
                        ot_data = fila.iloc[0].to_dict()
                        mostrar_detalle_jobplan(ot_data, solo_dropdown=True)

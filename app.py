        ...
        if archivo:
            ...
            for _, row in df.iterrows():
                ...
                if "IFSI" in location:
                    ...
                    # SOLO SEGUNDO TURNO
                    elif pmnum in PM_SEGUNDO_TURNO or codigo_sede in ["CLC", "CBA"]:

                        candidatos = [
                            t for t in tecnicos
                            if t["area"] == "SCI"
                            and str(t.get("turno_actual")).strip() == "2"
                            and t["id"] != tecnico_cba["id"]
                        ]
...
if modo == "General":
    ...
    if not df_prev.empty:
        ...
        # ---------------
            ...
            if st.button("Reemplazar foto final"):
                supabase.table("preventivos").update({
                    "evidencia_fin_url": None
                }).eq("numero_ot", ot_sel).execute()
            ...
            if st.session_state.confirmar_cierre:
                ...
                with col1:
                    if st.button("Sí, finalizar"):
                        if not evidencia_inicio:
                            st.error("Debes contar con foto inicial")
                        elif not evidencia_fin and not foto_fin:
                            st.error("Debes subir una foto de término")
                        else:

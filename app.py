...
def cargar_preventivos_dashboard():
    ...
    df["ANIO"] = df["schedfinish_dt"].dt.year
    df["MES_NUM"] = df["schedfinish_dt"].dt.month
...
if modo == "Admin":
    ...
    with tab0:
        ...
        if df_dash.empty:
            ...
            with col_f4:
                estatus_sel = multiselect_todos(
                    "Estatus",
                    df_dash["estatus"].dropna().unique(),
                    "dash_estatus_prev"
                )
            sedes_sel = multiselect_todos(
                "Sede",
                df_dash["sede"].dropna().unique(),
                "dash_sede_prev"
            )
            df_filtrado = df_dash[
                df_dash["MES"].isin(meses_sel)
                & df_dash["area"].isin(areas_sel)
                & df_dash["estatus"].isin(estatus_sel)
                & df_dash["sede"].isin(sedes_sel)
            ].copy()
            ...
            with sub1:
                ...
                fig_estado.update_layout(template="plotly_white", height=420)

                tendencia = (
                    df_filtrado[df_filtrado["MES_NUM"].notna()]
                    .groupby(["MES_NUM", "MES"])
                    .size()
                ...
                fig_tendencia = px.line(
                    ...
                    markers=True,
                    text="Total",
                    title="Tendencia mensual de preventivos"
                )
                fig_tendencia.update_traces(
                    textposition="top center", line=dict(width=4))
            ...
            with sub2:
                ...
                fig_area.update_layout(template="plotly_white", height=460)
            ...
            with sub3:
                abiertas = df_filtrado[
                    df_filtrado["estatus"].isin(["PENDIENTE", "EN PROCESO"])
                ]

                carga = (
                    abiertas.groupby(["tecnico", "area"])
                    .size()
                    .reset_index(name="OTs abiertas")
                    .sort_values("OTs abiertas", ascending=False)
                )
                if carga.empty:
                    ...
                    fig_carga = px.bar(
                        ...
                        orientation="h",
                        text="OTs abiertas",
                        title="Carga actual por tecnico"
                    )
            ...
            with sub4:
                ...
                columnas = [
                    c for c in columnas if c in df_filtrado.columns]
                df_detalle = df_filtrado[columnas].copy()

                def pintar_vencida(row):
                    if row.get("vencida", False):
                        return [

...
def generar_pdf_reporte_preventivos(df_reporte, firma_nombre):
    ...
    elementos.append(Paragraph("Reporte de Preventivos", title))
    ...
    tabla_resumen = Table(resumen, colWidths=[120, 120, 120, 120, 120])
    ...
    estado = df_reporte["estatus"].value_counts().reset_index()
    ...
    fig_estado.update_layout(template="plotly_white", width=700, height=360)

    img_estado = BytesIO(fig_estado.to_image(format="png", scale=2))
    elementos.append(Image(img_estado, width=360, height=185))
    elementos.append(Spacer(1, 14))

    columnas = ["numero_ot", "descripcion", "tecnico",
                "area", "sede", "schedfinish", "estatus"]

    def preparar_tabla(df, titulo):
        elementos.append(Paragraph(titulo, styles["Heading2"]))
        if df.empty:
            elementos.append(Paragraph("Sin registros", normal))
            elementos.append(Spacer(1, 12))
            return
        df_tabla = df[[c for c in columnas if c in df.columns]].copy()
        data = [[Paragraph(str(c), small) for c in df_tabla.columns]]

        for _, row in df_tabla.head(25).iterrows():
            data.append([
                Paragraph(str(row.get(c, ""))[:160], small)
        ...
        tabla.setStyle(TableStyle([
            ...
            ("FONTNAME", (0, 0), (-1, 0), "Helvetica-Bold"),
            ("FONTSIZE", (0, 0), (-1, -1), 0.4, colors.lightgrey),
            ("GRID", (0, 0), (-1, -1), 0.4, colors.lightgrey),
            ("VALIGN", (0, 0), (-1, -1), "TOP"),
        ]))
        elementos.append(tabla)
        elementos.append(Spacer(1, 14))
    df_vencidas = df_reporte[df_reporte["vencida"] == True].copy()

    hoy = date.today()
    df_proximas = df_reporte[
        df_reporte["estatus"].isin(["PENDIENTE", "EN PROCESO"])
        & df_reporte["schedfinish_dt"].notna()
        & ((df_reporte["schedfinish_dt"].dt.date - hoy).apply(lambda x: x.days).between(0, 3))
    ].copy(())

    preparar_tabla(df_vencidas, "OTs vencidas")
    preparar_tabla(df_proximas, "OTs proximas a vencer")

    elementos.append(Spacer(1, 18))
    elementos.append(
        Paragraph(f"Atentamente,<br/><b>{firma_nombre}</b>", normal))
        ...
        if df_dash.empty:
            ...
            with sub3:
                ...
                if realizadas_hist.empty:
                    ...
                    correos_reporte = st.text_input(
                        "Correos destino",
                        placeholder="hcedillo@acciona.com, jimartinezh@acciona.com, gabecerril@acciona.com, tamondragon@acciona.com",
                        key="correos_reporte_pdf"
                    )
                    ...
                    if st.button("Generar PDF y enviar", use_container_width=True):
                        destinatarios = [
                            c.strip()
                            for c in correos_reporte.split(",")
                            if c.strip()
                        ]
                        if not destinatarios:
                            st.error("Agregar al menos un correo destino.")
                        else:
        ...
        if not df_admin_prev.empty:
            ...
            # Mostrar evidencia si existe
                ...
                anotaciones_admin = st.text_area(
                    "Anotaciones",
                    value=str(anotaciones or ""),
                    height=150,
                    key=f"admin_anotaciones_{ot_sel_admin}"
                )
                ...
                with col1:
                    if st.button("Guardar cambios", key=f"guardar_admin_{ot_sel_admin}"):
                        ...
                        if foto_inicio_admin:
                            nombre_inicio = f"INICIO_ADMIN_{ot_sel_admin}_{uuid.uuid4().hex}.jpg"

                            supabase.storage.from_("evidencias").upload(
                                nombre_inicio,
                                foto_inicio_admin.getvalue()
                            )
                        ...
                        if foto_fin_admin:
                            nombre_fin = f"FIN_ADMIN_{ot_sel_admin}_{uuid.uuid4().hex}.jpg"

                            supabase.storage.from_("evidencias").upload(
                                nombre_fin,
                                foto_fin_admin.getvalue()
                            )

...
def subir_evidencia_comprimida(upload_file, prefijo, numero_ot):
    nombre_archivo = f"preventivos/{numero_ot}/{prefijo}_{uuid.uuid4().hex}.jpg"
    imagen_bytes = comprimir_imagen(upload_file)

    return subir_bytes_r2(
        nombre_archivo,
        imagen_bytes,
...
def crear_cliente_r2():
    endpoint_url = st.secrets.get(
        "R2_ENDPOINT_URL",
        f"https://{st.secrets['R2_ACCOUNT_ID']}.r2.cloudflarestorage.com"
    )
...
def subir_bytes_r2(nombre_archivo, archivo_bytes, content_type="image/jpeg"):
    s3 = crear_cliente_r2()
    ...
    return f"{st.secrets['R2_PUBLIC_URL'].rstrip('/')}/{nombre_archivo}"
...
def cargar_tabla_completa(nombre_tabla, columnas="*", page_size=1000):
    datos = []
    inicio = 0
...
def cargar_preventivos_dashboard():
    preventivos = cargar_tabla_completa("preventivos", "*")
    tecnicos_db = cargar_tabla_completa(
        "tecnicos",
        "id,nombre,area,activo"
    )
...
@st.cache_data(ttl=60)
def cargar_tareas_jobplan(jpnum):
    if not jpnum or pd.isna(jpnum) or str(jpnum).strip().lower() in ["", "none", "nan", "null"]:
        return []

    return (
        supabase.table("jobplan_tareas")
        .select("*")
        .eq("jpnum", str(jpnum).strip())
        .execute()
        .data
        or []
    )
...
def mostrar_detalle_jobplan(ot_data, solo_dropdown=False):
    descripcion = ot_data.get("descripcion")
    jpnum = ot_data.get("jpnum")

    if not solo_dropdown:
        st.markdown("---")
        st.subheader("Detalle de actividad / Job Plan")

    if descripcion and str(descripcion).strip().lower() not in ["nan", "none", "null", ""]:
        st.write(f"**Actividad:** {descripcion}")
    if not jpnum or str(jpnum).strip().lower() in ["nan", "none", "null", ""]:
    ...
    if not tareas:
        st.info("Esta OT no tiene tareas cargadas en el Job Plan")
        return

    df_tareas = pd.DataFrame(tareas)

    if "secuencia" in df_tareas.columns:
        df_tareas["secuencia_num"] = pd.to_numeric(
            df_tareas["secuencia"], errors="coerce"
        )
        df_tareas = df_tareas.sort_values("secuencia_num")
    ...
    with st.expander("Detalles de tareas", expanded=True):
        for _, row in df_tareas.iterrows():
            ...
            secuencia = str(secuencia).strip()
            tarea = str(tarea).strip()

            if secuencia and tarea:
                st.markdown(f"**{secuencia}** - {tarea}")
            elif tarea:
                st.markdown(tarea)
...
def obtener_nombre_supervisor(email):
    email = str(email or "").strip().lower()

    supervisores = {
        "hcedillo@acciona.com": "Hector Cedillo",
        "tamondragon@acciona.com": "Taissir Mondragon",
        "gabecerril@acciona.com": "Gabriel Becerril",
        "jimartinezh@acciona.com": "Ignacio Martinez"
    }
    return supervisores.get(email, email)
...
def generar_pdf_reporte_preventivos(df_reporte, firma_nombre):
    ...
    abiertos = df["estatus_norm"].isin(["PENDIENTE", "EN PROCESO"])

    df_pendientes = df[df["estatus_norm"] == "PENDIENTE"].copy()
    ...
    total = len(df)
    pendientes = len(df[df["estatus_norm"] == "PENDIENTE"])
    en_proceso = len(df[df["estatus_norm"] == "EN PROCESO"])
    realizadas = len(df[df["estatus_norm"] == "REALIZADO"])
    vencidas = len(df[df["vencida"] == True])
    ...
    estado = df["estatus_norm"].value_counts()
...
def asignar_fechas_visibles_preventivos(nuevos, dias_laborales=7):
    if not nuevos:
        return nuevos
...
def cargar_solicitudes_refacciones_supervisor(supervisor_email=None):
    return (
        ...
        .select("*")
        .or_("oculta_supervisor.is.null,oculta_supervisor.eq.false")
        .order("created_at", desc=True)
        .execute()
        .data or []
    )
...
def obtener_email_usuario_actual():
    usuario = st.session_state.get("user")
    ...
    if isinstance(usuario, dict):
        return str(usuario.get("email", "")).strip().lower()
    return str(getattr(usuario, "email", "")).strip().lower()


def subir_foto_refaccion_supabase(upload_file, solicitud_id):
    foto_bytes = comprimir_imagen(
        upload_file,
        max_px=700,
        calidad=45,
        max_kb=120
    )
    nombre_archivo = f"{solicitud_id}/foto_refaccion.jpg"

    supabase.storage.from_("refacciones").upload(
        nombre_archivo,
        foto_bytes,
        {
    ...
    foto_url = supabase.storage.from_(
        "refacciones").get_public_url(nombre_archivo)
    return foto_url, foto_bytes


def vista_supervisor_refacciones():
    st.subheader("Solicitud de refacciones")

    supervisor_email = obtener_email_usuario_actual()
    supervisor_nombre = obtener_nombre_supervisor(supervisor_email)

    if not supervisor_email:
        st.error("No se pudo identificar el usuario supervisor.")
        return
    ...
    with st.form("form_solicitud_refaccion", clear_on_submit=True):
        ...
        foto_refaccion = st.file_uploader(
            "Foto opcional de la pieza",
            type=["jpg", "jpeg", "png"],
            key="foto_refaccion_solicitud"
        )
    ...
    if enviar:
        ...
        if not all(str(c).strip() for c in campos):
            ...
            if foto_refaccion is not None:
                foto_url, foto_bytes = subir_foto_refaccion_supabase(
                    foto_refaccion,
                    solicitud_id
                )
    ...
    solicitudes = cargar_solicitudes_refacciones_supervisor()

    if isinstance(solicitudes, pd.DataFrame):
        df = solicitudes
    else:
        df = pd.DataFrame(solicitudes or [])

    if df.empty:
        st.info("No hay solicitudes de refacciones")
        return

    for _, sol in df.iterrows():
        with st.container(border=True):
            st.write(f"**Refacción:** {sol['refaccion']}")
            st.write(
            ...
            if (
                comentario_admin
                and not pd.isna(comentario_admin)
                and str(comentario_admin).strip().lower() not in ["", "nan", "none", "null"]
            ):
                st.info(f"Anotaciones admin: {comentario_admin}")

            foto_url = sol.get("foto_url")

            if (
                foto_url
                and not pd.isna(foto_url)
                and str(foto_url).strip().lower() not in ["", "nan", "none", "null"]
            ):
                st.image(foto_url, caption="Foto de referencia", width=300)

            if sol.get("estatus") == "FINALIZADA":
                if st.button("Quitar de pendientes", key=f"ocultar_ref_{sol.get('id')}"):
...
def agregar_refaccion_al_historico(solicitud):
    ...
    registro = {
        ...
        "created_by": solicitud.get("supervisor_email"),
        "rol": "supervisor",
        "supervisor_email": solicitud.get("supervisor_email"),
...
if st.session_state.pantalla == "admin_login":
    ...
    with column2:
        st.subheader("Login Admin / Supervisor")

        email = st.text_input("Correo")
        password = st.text_input("Contraseña", type="password")

        if st.button("Ingresar"):
            email_login = email.strip().lower()

            if (
                email_login == "ethanmijail22@gmail.com"
                and password == st.secrets.get("ADMIN_PASSWORD", "")
            ):
                st.session_state.user = SimpleNamespace(email=email_login)
...
def obtener_jobplan_ot(ot_data):
    for campo in ["jpnum", "jpnnum", "jobplan", "job_plan"]:
        valor = ot_data.get(campo)
        if valor not in [None, "", "None", "nan"]:
            return str(valor).strip()
    return ""


def es_checklist_arranque(ot_data):
    descripcion = str(ot_data.get("descripcion", "")).upper()
    jobplan = obtener_jobplan_ot(ot_data)

    return (
        jobplan == "1460244"
        or "CHECKLIST DE ARRANQUE" in descripcion
        or "CHECK LIST DE ARRANQUE" in descripcion
    )


@st.cache_data(ttl=30)
def cargar_checklist_preventivo(numero_ot, tipo_checklist, fecha_checklist):
    data = (
        supabase.table("checklist_preventivos")
        .select("*")
        .eq("numero_ot", numero_ot)
        .eq("tipo_checklist", tipo_checklist)
        .eq("fecha_checklist", fecha_checklist.isoformat())
        .execute()
        .data
        or []
    )
...
@st.cache_data(ttl=30)
def cargar_checklist_dias(numero_ot, tipo_checklist="ARRANQUE_COMEDORES"):
    return (
        supabase.table("checklist_preventivos_dias")
        .select("*")
        .eq("numero_ot", str(numero_ot))
        .eq("tipo_checklist", tipo_checklist)
        .execute()
        .data
        or []
...
def dia_checklist_finalizado(numero_ot, fecha_checklist, tipo_checklist="ARRANQUE_COMEDORES"):
    fecha_txt = fecha_checklist.isoformat()

    return any(
        str(r.get("fecha_checklist"))[:10] == fecha_txt
        and str(r.get("estatus_dia", "")).upper() == "FINALIZADO"
        for r in cargar_checklist_dias(numero_ot, tipo_checklist)
    )


def obtener_fecha_checklist_pendiente(numero_ot, fecha_base=None):
    hoy = date.today()
    fecha_base = fecha_base or hoy
    inicio = lunes_de_semana(fecha_base)

    for i in range(5):
        fecha = inicio + timedelta(days=i)

        if fecha <= hoy and not dia_checklist_finalizado(numero_ot, fecha):
            return fecha
    return None


def semana_checklist_completa(numero_ot, fecha_base):
    inicio = lunes_de_semana(fecha_base)

    return all(
        dia_checklist_finalizado(numero_ot, inicio + timedelta(days=i))
        for i in range(5)
    )


def guardar_cierre_dia_checklist_arranque(
    ot_data,
    fecha_checklist,
    registros_checklist,
    ...
    numero_ot = str(ot_data.get("numero_ot"))
    ahora = datetime.now(timezone.utc).isoformat()

    supabase.table("checklist_preventivos").upsert(
        registros_checklist,
        on_conflict="numero_ot,tipo_checklist,fecha_checklist,item"
    ).execute()

    supabase.table("checklist_preventivos_dias").upsert({
        "numero_ot": numero_ot,
        "tipo_checklist": "ARRANQUE_COMEDORES",
        "fecha_checklist": fecha_checklist.isoformat(),
        "dia_semana": nombre_dia(fecha_checklist),
        "estatus_dia": "FINALIZADO",
        "evidencia_inicio_url": evidencia_inicio_url,
        "evidencia_fin_url": evidencia_fin_url,
        "anotaciones": anotaciones,
        "usuario_email": obtener_email_usuario_actual(),
        "finalizado_at": ahora,
        "updated_at": ahora
    }, on_conflict="numero_ot,tipo_checklist,fecha_checklist").execute()

    cargar_checklist_preventivo.clear()
    cargar_checklist_dias.clear()

    if semana_checklist_completa(numero_ot, fecha_checklist):
        supabase.table("preventivos").update({
            "estatus": "REALIZADO",
            "fecha_cierre": ahora,
            "anotaciones": anotaciones,
            "evidencia_inicio_url": evidencia_inicio_url,
            "evidencia_fin_url": evidencia_fin_url
        }).eq("numero_ot", numero_ot).execute()

        return "OT_CERRADA"
    return "DIA_CERRADO"


def mostrar_checklist_arranque(ot_data, solo_lectura=False, fecha_checklist=None):
    numero_ot = str(ot_data.get("numero_ot", ""))
    tipo_checklist = "ARRANQUE_COMEDORES"

    if fecha_checklist is None:
        fecha_checklist = date.today()

    dia_semana = nombre_dia(fecha_checklist)

    guardados = cargar_checklist_preventivo(
        numero_ot,
        tipo_checklist,
        fecha_checklist
    )

    st.markdown(f"### Checklist de arranque a comedor - {dia_semana}")
    st.caption(f"OT: {numero_ot} | Fecha: {fecha_checklist.isoformat()}")

    if solo_lectura:
        filas = []
        for item in ITEMS_CHECKLIST_ARRANQUE:
            registro = guardados.get(item, {})
            filas.append({
                "Equipo": item,
                "Resultado": registro.get("resultado", "SIN REGISTRO"),
    ...
    if fecha_checklist.weekday() > 4:
        st.warning("Este checklist solo aplica de lunes a viernes")
        return

    registros = []

    for item in ITEMS_CHECKLIST_ARRANQUE:
        actual = guardados.get(item, {})
        resultado_actual = actual.get("resultado")
        opciones = ["OK", "REQUIERE ATENCION", "N/A"]
        index_actual = opciones.index(
            resultado_actual) if resultado_actual in opciones else None
        ...
        with col1:
            resultado = st.radio(
                ...
                index=index_actual,
                horizontal=True,
                key=f"resultado_{numero_ot}_{fecha_checklist}_{item}"
            )
        with col2:
            observaciones = st.text_input(
                "Observaciones",
                value=actual.get("observaciones", "") or "",
                key=f"obs_{numero_ot}_{fecha_checklist}_{item}",
...
def vista_tecnico_admin(solo_lectura=False):
    ...
    if not df_admin_prev.empty:
        ...
        if not fila.empty:
            ...
            ot_data = fila.iloc[0].to_dict()

            registros_checklist_arranque = []
            fecha_checklist_activa = None
            ot_es_checklist_arranque = es_checklist_arranque(ot_data)

            if ot_es_checklist_arranque:
                fecha_base = pd.to_datetime(
                    ot_data.get("fecha_visible_tecnico") or ot_data.get(
                        "created_at"),
                    errors="coerce"
                )

                fecha_base = date.today() if pd.isna(fecha_base) else fecha_base.date()

                fecha_checklist_activa = obtener_fecha_checklist_pendiente(
                    ot_data.get("numero_ot"),
                    fecha_base
                )

                if fecha_checklist_activa is None:
                    st.success(
                        "Checklist del día ya finalizado. Volverá a aparecer el siguiente día."
                    )
                else:
                    if fecha_checklist_activa < date.today():
                        st.warning(
                            f"Checklist atrasado: {nombre_dia(fecha_checklist_activa)}"
                            f"{fecha_checklist_activa.isoformat()}"
                        )
                    else:
                        st.info(
                            f"Checklist de hoy: {nombre_dia(fecha_checklist_activa)}"
                            f"{fecha_checklist_activa.isoformat()}"
                        )

                    registros_checklist_arranque = mostrar_checklist_arranque(
                        ot_data,
                        solo_lectura=solo_lectura,
                        fecha_checklist=fecha_checklist_activa
                    )
            else:
                mostrar_detalle_jobplan(ot_data)
                ...
                with col2:
                    if st.button("Cerrar OT con evidencias", key=f"cerrar_admin_{ot_sel_admin}"):
                        ...
                        if ot_es_checklist_arranque:
                            if fecha_checklist_activa is None:
                                st.info(
                                    "No hay checklist pendiente para cerrar hoy.")
                                st.stop()

                            incompletos = [
                                r["item"]
                                for r in registros_checklist_arranque
                                if not r.get("resultado")
                            ]

                            if incompletos:
                                st.error(
                                    "Completa todos los puntos del checklist antes de cerrar")
                                st.stop()
...
if modo == "Admin":
    ...
    if seccion_admin == "Inicio":
        ...
        if df_dash.empty:
            ...
            with sub1:
                ...
                df_diario = df_dash.copy()

                if "fecha_visible_tecnico" not in df_diario.columns:
                    df_diario["fecha_visible_tecnico"] = None

                df_diario["fecha_visible_dt"] = pd.to_datetime(
                    df_diario["fecha_visible_tecnico"],
                    errors="coerce"
                )

                df_diario = df_diario[
                    df_diario["fecha_visible_dt"].notna()
                    & df_diario["area"].isin(areas_sel)
                ].copy()

                if anio_sel != "TODOS":
                    df_diario = df_diario[
                        df_diario["fecha_visible_dt"].dt.year == anio_sel
                    ]

                df_diario["MES_VISIBLE"] = df_diario["fecha_visible_dt"].dt.month.map(
                    MESES_CORTOS)
                df_diario = df_diario[df_diario["MES_VISIBLE"].isin(meses_sel)]

                if isinstance(rango_semana, tuple) and len(rango_semana) == 2:
                    df_diario = df_diario[
                        (df_diario["fecha_visible_dt"].dt.date >=
                         inicio_semana)
                        & (df_diario["fecha_visible_dt"].dt.date <= fin_semana)
                    ]

                df_diario["dia"] = df_diario["fecha_visible_dt"].dt.date

                df_diario["fecha_cierre_dia"] = pd.to_datetime(
                    df_diario["fecha_cierre_dt"],
                    errors="coerce"
                ).dt.date

                df_diario["realizada_en_tiempo"] = (
                    df_diario["estatus"].eq("REALIZADO")
                    & df_diario["fecha_cierre_dia"].notna()
                    & (df_diario["fecha_cierre_dia"] == df_diario["dia"])
                )

                df_diario["realizada_a_destiempo"] = (
                    df_diario["estatus"].eq("REALIZADO")
                    & df_diario["fecha_cierre_dia"].notna()
                    & (df_diario["fecha_cierre_dia"] != df_diario["dia"])
                )
                ...
                resumen_diario = (
                    df_diario.groupby("dia")
                    .agg(
                        programadas=("numero_ot", "count"),
                        realizadas=("realizada_en_tiempo", "sum"),
                        realizadas_destiempo=("realizada_a_destiempo", "sum")
                    )
                    .reset_index()
                )

                dias_semana = pd.DataFrame({
                    "dia": pd.date_range(inicio_semana, fin_semana, freq="D").date
                })

                resumen_diario = dias_semana.merge(
                    resumen_diario,
                    on="dia",
                    how="left"
                ).fillna(0)

                resumen_diario["programadas"] = resumen_diario["programadas"].astype(
                    int)
                resumen_diario["realizadas"] = resumen_diario["realizadas"].astype(
                    int)
                resumen_diario["pendientes"] = (
                    resumen_diario["programadas"] -
                    resumen_diario["realizadas"] -
                    resumen_diario["realizadas_destiempo"]
                )
                resumen_diario["cumplimiento"] = resumen_diario.apply(
                    lambda r: round(
                        (r["realizadas"] / r["programadas"]) * 100, 1)
                    if r["programadas"] > 0 else 0,
                    axis=1
                )
                resumen_diario["dia_str"] = pd.to_datetime(
                    resumen_diario["dia"]
                ).dt.strftime("%Y-%m-%d")

                resumen_diario["no_cumplido"] = resumen_diario.apply(
                    lambda r: round(100 - r["cumplimiento"], 1)
                    if r["programadas"] > 0 else 0,
                    axis=1
                )

                resumen_grafica = resumen_diario.melt(
                    id_vars=[
                        "dia",
                        "dia_str",
                        "programadas",
                        "realizadas",
                        "realizadas_destiempo",
                        "pendientes"
                    ],
                    value_vars=["cumplimiento", "no_cumplido"],
                    var_name="tipo",
                    value_name="porcentaje"
                )

                resumen_grafica["tipo"] = resumen_grafica["tipo"].replace({
                    "cumplimiento": "Cumplido",
                    "no_cumplido": "No cumplido"
                })

                fig_cumplimiento_diario = px.bar(
                    resumen_grafica,
                    x="dia",
                    y="porcentaje",
                    color="tipo",
                    barmode="group",
                    text="porcentaje",
                    hover_data=["programadas",
                                "realizadas",
                                "realizadas_destiempo",
                                "pendientes"
                                ],
                    custom_data=["dia_str"],
                    title="Cumplimiento diario de preventivos",
                    labels={
                        "dia": "Dia",
                ...
                fig_cumplimiento_diario.update_traces(
                    texttemplate="%{text:.1f}%",
                    textposition="outside"
                )

                fig_cumplimiento_diario.update_layout(
                    template="plotly_white",
                    height=450,
                ...
                with col_area:
                    st.plotly_chart(fig_area_resumen, use_container_width=True, config={
                                    "displayModeBar": False})
                evento_dia = st.plotly_chart(
                    fig_cumplimiento_diario,
                    use_container_width=True,
                    key="grafica_cumplimiento_diario",
                    on_select="rerun",
                    selection_mode="points"
                )

                puntos = evento_dia.selection.points if evento_dia.selection else []

                if puntos:
                    dia_seleccionado = puntos[0]["customdata"][0]
                    dia_sel = pd.to_datetime(dia_seleccionado).date()

                    st.markdown("### Detalle del dia seleccionado")

                    df_detalle_dia = df_diario[
                        df_diario["dia"] == dia_sel
                    ].copy()

                    if df_detalle_dia.empty:
                        st.info("No hay preventivos programadas para este día.")
                    else:
                        df_detalle_dia["fecha_cierre_dia"] = pd.to_datetime(
                            df_detalle_dia["fecha_cierre_dt"],
                            errors="coerce",
                            utc=True
                        ).dt.tz_convert("America/Mexico_City").dt.date

                        df_detalle_dia["fecha_cierre_solo"] = pd.to_datetime(
                            df_detalle_dia["fecha_cierre_dt"],
                            errors="coerce",
                            utc=True
                        ).dt.tz_convert("America/Mexico_City").dt.strftime("%Y-%m-%d")

                        df_detalle_dia["fecha_cierre_solo"] = (
                            df_detalle_dia["fecha_cierre_solo"]
                            .fillna("")
                            .replace("NaT", "")
                        )
                        df_detalle_dia["estatus"] = (
                            df_detalle_dia["estatus"]
                            .fillna("")
                            .astype(str)
                            .str.upper()
                    ...
                    df_detalle_dia["seguimiento"] = "PENDIENTE"

                    df_detalle_dia.loc[
                        (df_detalle_dia["estatus"] == "REALIZADO")
                        & df_detalle_dia["fecha_cierre_dia"].notna()
                        & (df_detalle_dia["fecha_cierre_dia"] == df_detalle_dia["dia"]),
                        "seguimiento"
                    ] = "REALIZADA EN TIEMPO"
                    ...
                    df_detalle_dia.loc[
                        (df_detalle_dia["estatus"].isin(
                            ["PENDIENTE", "EN PROCESO"]))
                        & (df_detalle_dia["dia"] < date.today()),
                        "seguimiento"
                    ] = "ATRASADA"

                    resumen_dia_sel = resumen_diario[
                        resumen_diario["dia_str"] == dia_seleccionado
                    ].iloc[0]

                    programadas_dia = int(resumen_dia_sel["programadas"])
                    realizadas_tiempo_dia = int(resumen_dia_sel["realizadas"])
                    realizadas_destiempo_dia = int(
                        resumen_dia_sel["realizadas_destiempo"])
                    no_cumplidas_dia = programadas_dia - realizadas_tiempo_dia

                    c1, c2, c3, c4 = st.columns(4)

                    c1.metric("Programadas", programadas_dia)
                    c2.metric("Realizadas en tiempo", realizadas_tiempo_dia)
                    c3.metric("Realizadas a destiempo",
                              realizadas_destiempo_dia)
                    c4.metric("No cumplidas", no_cumplidas_dia)
                    ...
                    columnas_detalle = [
                        ...
                        if c in df_detalle_dia.columns
                    ]

                    st.dataframe(
                        df_detalle_dia[columnas_detalle].rename(columns={
                            "fecha_cierre_solo": "fecha_cierre"
                        }),
                        use_container_width=True,
                        hide_index=True
                    )
                else:
                    st.caption("Haz click en una barra para ver detalle")
        ...
        if archivo and (cargar_prev or actualizar_fechas or actualizar_sedes):
            try:
                ...
                for _, row in df.iterrows():
                    ...
                    es_clc = (
                        ("CLC" in descripcion or "CLC" in str(sede).upper())
                        and "IFSI" not in location
                    )
                ...
                if cargar_prev:
                    ...
                    existentes = cargar_tabla_completa(
                        "preventivos",
                        "numero_ot"
                    )
                    ...
                    if nuevos:
                        nuevos = asignar_fechas_visibles_preventivos(nuevos)
    ...
    if seccion_admin == "Auditoria":
        ...
        rango_auditoria = st.date_input(
            "Rango de fechas de cierre / evidencia",
            value=(inicio_mes, hoy),
            key="rango_auditoria_evidencias"
        )
        ...
        if termino:
            ...
            if not isinstance(rango_auditoria, tuple) or len(rango_auditoria) != 2:
                st.info("Selecciona fecha inicial y fecha final.")
            else:
                fecha_ini, fecha_fin = rango_auditoria
                ...
                resultados = supabase.table("preventivos") \
                    ...
                    .lt("created_at", fin_utc.isoformat()) \
                    .or_(
                ...
                if df_resultados.empty:
                    ...
                    ot_sel_audit = st.selectbox(
                        ...
                        df_resultados["numero_ot"].tolist(),
                        key="auditoria_ot"
                    )
                    ...
                    if not fila.empty:
                        ...
                        if es_checklist_arranque(ot_data):
                            fecha_base = pd.to_datetime(
                                ot_data.get("fecha_visible_tecnico") or ot_data.get(
                                    "created_at"),
                                errors="coerce"
                            ...
                            if pd.isna(fecha_base):
                                fecha_base = pd.Timestamp(date.today())

                            inicio_semana_checklist = fecha_base.date() - timedelta(
                                days=fecha_base.date().weekday()
                            )

                            opciones_dias = {
                                f"{nombre_dia(inicio_semana_checklist + timedelta(days=i))} - {(inicio_semana_checklist + timedelta(days=i)).isoformat()}":
                                inicio_semana_checklist + timedelta(days=i)
                                for i in range(5)
                            }

                            dia_sel_label = st.selectbox(
                                "Selecciona dia del checklist",
                                list(opciones_dias.keys()),
                                key=f"dia_checklist_auditoria_{ot_data.get('numero_ot')}"
                            )

                            fecha_sel = opciones_dias[dia_sel_label]

                            mostrar_checklist_arranque(
                                ot_data,
                                solo_lectura=True,
...
if modo == "General":
    ...
    if not df_prev.empty:
        if "fecha_visible_tecnico" not in df_prev.columns:
            df_prev["fecha_visible_tecnico"] = None

        df_prev["fecha_visible_dt"] = pd.to_datetime(
            df_prev["fecha_visible_tecnico"],
            errors="coerce"
        )
        ...
        df_prev = df_prev[
            df_prev["fecha_visible_dt"].isna()
            | (df_prev["fecha_visible_dt"].dt.date <= hoy)
        ].copy()

        df_prev["estado_diario"] = "HOY"
        ...
        df_prev.loc[
            df_prev["estatus"].isin(["PENDIENTE", "EN PROCESO"])
            & df_prev["fecha_visible_dt"].notna()
            & (df_prev["fecha_visible_dt"].dt.date < hoy),
            "estado_diario"
        ] = "ATRASADA"

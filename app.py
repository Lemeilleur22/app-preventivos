...
if modo == "Admin":
    ...
    with tab6:
        ...
        if archivo_roles:
            ...
            st.dataframe(df_raw.iloc[0:20, 0:10])

            fila_header = None

            for i, row in df_raw.iterrows():
                texto_fila = " ".join(
                    [str(x).strip().upper() for x in row if pd.notna(x)]
                )

                if "NOMBRE" in texto_fila and "TURNO" in texto_fila:
                    fila_header = i
                    break
            ...
            df_roles.columns = df_roles.columns.astype(
